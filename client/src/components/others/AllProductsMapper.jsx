  ...
  return (
    <>
      ...
      <Pagination className="pagination-div">
        ...
        />
        {Array.from({ length: Math.ceil(productsLength / ITEMS_PER_PAGE) }).map(
          (_, index) => (
            <Pagination.Item
              key={index}
              onClick={() => paginate(index + 1)}
              className="pagination-count-button"
            >
              {index + 1}
            </Pagination.Item>
          )
        ...
        <Pagination.Next
          onClick={nextPage}
          disabled={currentPage === Math.ceil(productsLength / ITEMS_PER_PAGE)}
          className="pagination-previous-button"
        />
      </Pagination>
    </>
