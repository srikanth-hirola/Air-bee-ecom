export const Row2Products = {
    main: {
        header: {
            styles: {

            }
        }
    },
    productimages: {
        images:{
         content: {
             items: {
                 structure: {
                     img: {
                         public_id: "",
                         url: ""
                     },
                     Title:{
                         text:""
                     },
                     SubText:{
                         text:""
                     }
                 },
                 items: [
                     {
                         img: {
                             public_id: "",
                             url: "/DronesHomepage/row-2-2.png"
                         },
                         Title:{
                             text:"Drone Parts"
                         },
                         SubText:{
                             text:"From USD 19"
                         }

                     },
                     {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-3.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-4.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-5.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-6.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-7.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-8.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row-2-9.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
          
                 ]
             }
         }
        }
     },
}
export const Row3Products = {
    main: {
        header: {
            styles: {

            }
        }
    },
    productimages: {
        images:{
         content: {
             items: {
                 structure: {
                     img: {
                         public_id: "",
                         url: ""
                     },
                     Title:{
                         text:""
                     },
                     SubText:{
                         text:""
                     }
                 },
                 items: [
                     {
                         img: {
                             public_id: "",
                             url: "/DronesHomepage/row3-1.png"
                         },
                         Title:{
                             text:"Drone Parts"
                         },
                         SubText:{
                             text:"From USD 19"
                         }

                     },
                     {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-2.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-3.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-4.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-5.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-6.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-7.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/row3-8.png"
                        },
                        Title:{
                            text:"Drone Parts"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
          
                 ]
             }
         }
        }
     },
}

export const categoriescomponent = {
    main: {
        header: {
            styles: {

            }
        }
    },
    categoties: {
        images:{
         content: {
             items: {
                 structure: {
                     img: {
                         public_id: "",
                         url: ""
                     },
                     Title:{
                         text:""
                     },
                     SubText:{
                         text:""
                     }
                 },
                 items: [
                     {
                         img: {
                             public_id: "",
                             url: "/DronesHomepage/categories1.png"
                         },
                         Title:{
                             text:"Development Brands"
                         },
                         SubText:{
                             text:"From USD 19"
                         }

                     },
                     {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories2.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories3.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories4.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories5.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories6.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"From USD 19"
                        }

                    },
                 
                 ]
             }
         }
        }
     },
}
export const RecommendedItems = {
    main: {
        header: {
            styles: {

            }
        }
    },
    RecommendedItemscontent: {
        images:{
         content: {
             items: {
                 structure: {
                     img: {
                         public_id: "",
                         url: ""
                     },
                     Title:{
                         text:""
                     },
                     SubText:{
                         text:""
                     }
                 },
                 items: [
                     {
                         img: {
                             public_id: "",
                             url: "/DronesHomepage/categories1.png"
                         },
                         Title:{
                             text:"Development Brands"
                         },
                         SubText:{
                             text:"100A Multirotor ESC Power Distribution"
                         }

                     },
                     {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories2.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories3.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories1.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                    {
                       img: {
                           public_id: "",
                           url: "/DronesHomepage/categories2.png"
                       },
                       Title:{
                           text:"Development Brands"
                       },
                       SubText:{
                           text:"100A Multirotor ESC Power Distribution"
                       }

                   },
                   {
                       img: {
                           public_id: "",
                           url: "/DronesHomepage/categories3.png"
                       },
                       Title:{
                           text:"Development Brands"
                       },
                       SubText:{
                           text:"100A Multirotor ESC Power Distribution"
                       }

                   },
                   {
                       img: {
                           public_id: "",
                           url: "/DronesHomepage/categories4.png"
                       },
                       Title:{
                           text:"Development Brands"
                       },
                       SubText:{
                           text:"100A Multirotor ESC Power Distribution"
                       }

                   },
                   {
                       img: {
                           public_id: "",
                           url: "/DronesHomepage/categories5.png"
                       },
                       Title:{
                           text:"Development Brands"
                       },
                       SubText:{
                           text:"100A Multirotor ESC Power Distribution"
                       }

                   },
                   {
                       img: {
                           public_id: "",
                           url: "/DronesHomepage/categories6.png"
                       },
                       Title:{
                           text:"Development Brands"
                       },
                       SubText:{
                           text:"100A Multirotor ESC Power Distribution"
                       }

                   },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories4.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories5.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                    {
                        img: {
                            public_id: "",
                            url: "/DronesHomepage/categories6.png"
                        },
                        Title:{
                            text:"Development Brands"
                        },
                        SubText:{
                            text:"100A Multirotor ESC Power Distribution"
                        }

                    },
                 
                 ]
             }
         }
        }
     },
}

export const brands={
    main: {
        header: {
            styles: {

            }
        }
    },
    brandlogos:{
        logos:{
            content:{
                items:[
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands1.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands2.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands3.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands4.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands5.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands6.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands7.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands8.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands9.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands10.png"
                        }
                    }, {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands11.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/brands12.png"
                        }
                    },
                ]
            }
        }
    }
}


export const Footer = {
    main: {
        header: {
            styles: {

            }
        }
    },

    FooterLogo:{
        logo:{
            content:{
                img:{
                    public_id:"",
                    url:"/images/logo.png"
                }
            }
        }
    },
    CompanyDescription:{
        description:{
            content:{
                text:"Crafting spaces where elegance meets comfort every corner."
            }
        }
    },

    FooterItems: {
        navigate: {
            content: {
                items: {
                    structure: {
                        Title: '',
                        url: ''
                    },
                    drop: true,
                    items: [
                        {
                            Title: 'Quick Links',
                            url: '',
                            drop: [
                                {
                                    SubTitle: 'Home',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'About Us',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Shop',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Blogs',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Contact Us',
                                    url: 'http://localhost:3000'
                                },
                               
                               
                            ]
                        },
                        {
                            Title: 'Customer Info',
                            url: '',
                            drop: [
                                {
                                    SubTitle: 'Payment',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Delivery',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Order Tracking',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Exchange & Returns',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'T & C',
                                    url: 'http://localhost:3000'
                                }
                            ]
                        },
                       
                        {
                            Title: 'Collections',
                            url: '',
                            drop: [
                                {
                                    SubTitle: 'New Incomes',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Bestsellers',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Sale',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Testimonials',
                                    url: 'http://localhost:3000'
                                },
                                {
                                    SubTitle: 'Sitemap',
                                    url: 'http://localhost:3000'
                                },
                                // {
                                //     SubTitle: 'Q & A',
                                //     url: 'http://localhost:3000'
                                // },
                            ]
                        },
                        {
                            Title: 'Customer Support',
                            url: '',
                            drop: [
                                {
                                    SubTitle: 'Address: Air Bee, NO 7, 4th floor, 1st Cross Rd, Balaji Layout, AMCO Colony, Koti Hosahalli, Bengaluru, Karnataka 560092',
                                    url: 'http://localhost:3000'
                                },
                                // {
                                //     SubTitle: 'Delivery',
                                //     url: 'http://localhost:3000'
                                // },
                                // {
                                //     SubTitle: 'Order Tracking',
                                //     url: 'http://localhost:3000'
                                // },
                                // {
                                //     SubTitle: 'Exchange & Returns',
                                //     url: 'http://localhost:3000'
                                // },
                                // {
                                //     SubTitle: 'T & C',
                                //     url: 'http://localhost:3000'
                                // }
                            ]
                        },
                        
                    ]
                }
            }
        },
    },

    FooterNewsletter: {
        Newsletter:{
            content:{
                img:{
                    public_id:"",
                    url:"/images/SVG-4.png"
                },
                Title:{
                    text:"Newsletter signup"
                },
                SubText:{
                   
                    text:"Be the First to know about our new arrivals and exclusive offers !"
                },
                button:{
                    link:{
                        title:"Sign me up",
                        url:""
                    }
                }
            }
        }
    },
    SocialMedia:{
        Links:{
            content:{
                img:[
                    {
                        public_id:"",
                        url:"/DronesHomepage/fb.png",
                        link:"https://www.facebook.com/"
                    },
                    {
                        public_id:"",
                        url:"/DronesHomepage/linkedin3.png",
                        link:"https://www.linkedin.com/"
                    },
                    {
                        public_id:"",
                        url:"/DronesHomepage/instagram3.png",
                        link:"https://www.instagram.com/"
                    },
                    {
                        public_id:"",
                        url:"/DronesHomepage/twitter3.png",
                        link:"https://twitter.com/"
                    },
                    {
                        public_id:"",
                        url:"/DronesHomepage/youtube3.png",
                        link:"https://www.youtube.com/"
                    },
                ]
            }
        }
    },
    Copyrightscontent:{
        Copyrights:{
            content:{
                text:"© Copyright 2023 Dream-Theme. All rights reserved."
            }
        }
    },

    paymentOptions:{
        images:{
            content:{
                img:[
                    {
                        public_id:"",
                        url:"/images/symbol.png"
                    },
                    {
                        public_id:"",
                        url:"/images/symbol-1.png"
                    },
                    {
                        public_id:"",
                        url:"/images/symbol-2.png"
                    },
                    {
                        public_id:"",
                        url:"/images/symbol-3.png"
                    },
                    {
                        public_id:"",
                        url:"/images/symbol-4.png"
                    },
                ]
            }
        }
    }
}

export const Services = {
    main: {
        header: {
            styles: {

            }
        }
    },
    servicesIcon:{
        icon:{
            content:{
                items:[
                    
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/icon1.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/icon2.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/icon3.png"
                        }
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/icon4.png"
                        }
                    },
                    
                    

                ]
                
            }
        }
    },

    ServicesContent:{
        servicesimages:{
            content:{
                items:{
                    structure:{
                        img:{
                            public_id:"",
                            url:""
                        },
                        SubText:{
                            text:""
                        }
                    },
                    items:[
                        {
                            img:{
                                public_id:"",
                                url:"/DronesHomepage/services1.png"
                            },
                            SubText:{
                                text:"Automotive Industry"
                            }

                        },
                        {
                            img:{
                                public_id:"",
                                url:"/DronesHomepage/services2.png"
                            },
                            SubText:{
                                text:"Healthcare Sector"
                            }

                        },
                        {
                            img:{
                                public_id:"",
                                url:"/DronesHomepage/services3.png"
                            },
                            SubText:{
                                text:"Smart Home Technology"
                            }

                        },
                        {
                            img:{
                                public_id:"",
                                url:"/DronesHomepage/services4.png"
                            },
                            SubText:{
                                text:"Industrial Automation"
                            }

                        },
                    ]
                },

            }
        }
    }

}
export const flags = {
    main: {
        header: {
            styles: {

            }
        }
    },
    countryflags:{
        flagimages:{
            content:{
                structure:{
                    img:{
                        public_id:"",
                        url:""
                    },
                    title:{
                        text:""
                    },
                    SubText:{
                        text:""
                    }
                },
                items:[
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag1.png"
                        },
                        title:{
                            text:"Arabic Emirates"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag2.png"
                        },
                        title:{
                            text:"Australia"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag3.png"
                        },
                        title:{
                            text:"United States"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag4.png"
                        },
                        title:{
                            text:"Russia"
                        },
                        SubText:{
                            text:"shopname.ru"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag7.png"
                        },
                        title:{
                            text:"France"
                        },
                        SubText:{
                            text:"shopname.fr"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag8.png"
                        },
                        title:{
                            text:"Arabic Emirates"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag7.png"
                        },
                        title:{
                            text:"China"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag10.png"
                        },
                        title:{
                            text:"Great Britain"
                        },
                        SubText:{
                            text:"shopname.co.uk"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag8.png"
                        },
                        title:{
                            text:"Arabic Emirates"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                    {
                        img:{
                            public_id:"",
                            url:"/DronesHomepage/flag7.png"
                        },
                        title:{
                            text:"China"
                        },
                        SubText:{
                            text:"shopname.ae"
                        } 
                    },
                ]
            }
        }
    }

}