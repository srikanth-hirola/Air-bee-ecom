...
export const FormInputs = ({ blogData, setBlogData, _id }) => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const { data } = await axios.get(`${server}/blogs/category`)
    ...
    const handleMultipleImages = (e) => {
        ...
        const processImage = (file, index) => {
            if (file.size <= maxSizeInBytes) {
                ...
                toast.error(`File "${file.name}" exceeds the size limit of 200 KB.`);
            }
        };
    ...
    const handleAddCategory = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`${server}/blogs/category`, { category }, {
    ...
    const handleCloudinaryImageDelete = async (public_id, _id) => {
        try {
            await axios.delete(`${server}/blogs/delete-Img/${public_id}/${_id}`, {
                withCredentials: true,
            })
