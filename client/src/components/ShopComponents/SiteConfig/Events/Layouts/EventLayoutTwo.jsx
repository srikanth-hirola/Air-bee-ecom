...
export const EventLayoutTwo = ({ setLogoLoading, active }) => {
            ...
            {active === 1 &&
                <>
                    <div className='w-[95%] mx-auto mb-2 flex justify-between'>
                        ...
                        <button className='rounded bg-green-400 p-2 text-white cursor-pointer' onClick={() => saveLayoutContent(eventLayountTwoSelectedEvents, "eventLayountTwoSelectedEvents", setLogoLoading)}>
                            <i class="fa-solid fa-floppy-disk fa-1x"></i> Save
                        </button>
                    </div>

                    {allEvents?.length > 0 && <SelectEventModal onHide={() => setSelectedContentModalShow(false)} show={selectedContentModalShow} setModalShow={setSelectedContentModalShow} setlayout={setEventLayoutTwoSelectedEvents} layout={eventLayountTwoSelectedEvents} uploadedEvents={allEvents} name={"eventLayountTwoSelectedEvents"} multiple={true} />}
                </>
            }
        </>
    )
