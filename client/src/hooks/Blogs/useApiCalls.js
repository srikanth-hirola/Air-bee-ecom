...
export const useApiCalls = () => {
    ...
    const handleCloudinaryImageDelete = async (public_id, _id, data, setData) => {
        try {
            await axios.delete(`${url}admin/delete-Img/${public_id}/${_id}`, { withCredentials: true })
    ...
    const handleAddCategory = async (e, dropData, setDropData, category, endpoint, name) => {
        e.preventDefault();
        try {
            await axios.post(`${url}${endpoint}`, { [name]: category }, { withCredentials: true })
    ...
    const updateData = async (e, data, setLoading, endpoint, navPath, message) => {
        ...
        setLoading(true)
        try {
            await axios.put(`${url}${endpoint}`, { data }, { withCredentials: true })
            navigate(navPath);
            toast.success(message)
            setLoading(false)
        } catch (e) {
    ...
    const deleteData = async (e, setLoading, endpoint, message) => {
        ...
        try {
            setLoading(true)
            await axios.delete(`${url}${endpoint}`, { withCredentials: true })
